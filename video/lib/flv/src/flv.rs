...
use crate::{
    define::Flv, AacPacket, AacPacketType, Av1Packet, AvcPacket, AvcPacketType, EnhancedPacket,
    EnhancedPacketType, FlvDemuxerError, FlvHeader, FlvTag, FlvTagAudioData, FlvTagData,
    FlvTagType, FlvTagVideoData, FrameType, HevcPacket, SoundCodecId, SoundRate, SoundSize,
    SoundType, VideoCodecId, VideoFourCC,
};
    ...
    pub fn demux(tag_type: u8, data: Bytes) -> Result<Self, FlvDemuxerError> {
        ...
        match FlvTagType::from_u8(tag_type) {
            Some(FlvTagType::Audio) => {
                ...
                let sound_rate = SoundRate::from_u8(sound_rate)
                    .ok_or_else(|| FlvDemuxerError::InvalidSoundRate(sound_rate))?;

                let sound_size = (flags & 0b0000_0010) >> 1;
                let sound_size = SoundSize::from_u8(sound_size)
                    .ok_or_else(|| FlvDemuxerError::InvalidSoundSize(sound_size))?;

                let sound_type = flags & 0b0000_0001;
                let sound_type = SoundType::from_u8(sound_type)
                    .ok_or_else(|| FlvDemuxerError::InvalidSoundType(sound_type))?;
            ...
            Some(FlvTagType::Video) => {
                ...
                let frame_type = FrameType::from_u8(frame_type)
                    .ok_or_else(|| FlvDemuxerError::InvalidFrameType(frame_type))?;
...
impl FlvTagVideoData {
    ...
    pub fn demux_enhanced(
        ...
        match (video_codec, packet_type) {
            (VideoFourCC::Av1, EnhancedPacketType::SequenceStart) => {
                Ok(Self::Enhanced(EnhancedPacket::Av1(
    ...
    pub fn demux(
        ...
        match AvcPacketType::from_u8(avc_packet_type) {
            Some(AvcPacketType::SeqHdr) => {
                ...
                Ok(Self::SequenceHeader(AVCDecoderConfigurationRecord::demux(
                    reader,
                )?))
            }
