use super::{define::RtmpCommand, errors::SessionError};
use crate::{
    channels::{ChannelData, DataProducer, PublishRequest, UniqueID},
    chunk::{ChunkDecoder, ChunkEncoder, CHUNK_SIZE},
    handshake,
    handshake::{HandshakeServer, ServerHandshakeState},
    messages::{MessageParser, RtmpMessageData},
    netconnection::NetConnection,
    netstream::NetStreamWriter,
    protocol_control_messages::ProtocolControlMessagesWriter,
    user_control_messages::EventMessagesWriter,
    PublishProducer,
};
use amf0::Amf0Value;
use bytes::Bytes;
...
pub struct Session<S: AsyncReadWrite> {
    ...
    /// However the stream key is not required to be the same for each stream you publish / play
    /// Traditionally we only publish a single stream per RTMP connection,
    /// However we can publish multiple streams per RTMP connection (using different stream keys)
    /// and or play multiple streams per RTMP connection (using different stream keys) as per the RTMP spec.
    ...
    /// this flag is used to indicate that we have data ready to parse and we should not read more data from the stream
    skip_read: bool,

    /// This is used to read the data from the stream and convert it into rtmp messages
    ...
    /// when the publisher connects and tries to publish a stream, we need to send a publish request to the server
...
impl<S: AsyncReadWrite> Session<S> {
    ...
    /// The result of the return value will be true if all publishers have disconnected
    /// If any publishers are still connected, the result will be false
    /// This can be used to detect non-graceful disconnects (ie. the client crashed)
    pub async fn run(&mut self) -> Result<bool, SessionError> {
        ...
        // However most clients just disconnect without cleanly stopping the subscrition streams (play streams)
        // So we just check that all publishers have disconnected cleanly
        Ok(!self.is_publishing)
    }

    /// This is the first stage of the session
    /// It is used to do the handshake with the client
    /// The handshake is the first thing that happens when you connect to an rtmp server
    async fn do_handshake(
        &mut self,
        handshaker: &mut HandshakeServer,
    ...
    async fn process_messages(
        ...
        match rtmp_msg {
            ...
            RtmpMessageData::AudioData { data } => {
                self.on_data(stream_id, ChannelData::Audio { timestamp, data })
                    .await?;
            }
            RtmpMessageData::VideoData { data } => {
                self.on_data(stream_id, ChannelData::Video { timestamp, data })
                    .await?;
            }
    ...
    /// on_data is called when we receive a data message from the client (a published_stream)
    /// Such as audio, video, or metadata
    ...
    /// on_amf0_command_message is called when we receive an AMF0 command message from the client
    /// We then handle the command message
    async fn on_amf0_command_message(
        ...
        match cmd {
            RtmpCommand::Connect => {
                self.on_command_connect(transaction_id, stream_id, obj, others)
                    .await?;
            }
            RtmpCommand::CreateStream => {
                self.on_command_create_stream(transaction_id, stream_id, obj, others)
                    .await?;
            }
            RtmpCommand::DeleteStream => {
                self.on_command_delete_stream(transaction_id, stream_id, obj, others)
                    .await?;
            }
            RtmpCommand::Play => {
            ...
            RtmpCommand::Publish => {
                self.on_command_publish(transaction_id, stream_id, obj, others)
                    .await?;
            }
    ...
    /// on_set_chunk_size is called when we receive a set chunk size message from the client
    /// We then update the chunk size of the unpacketizer
    ...
    /// on_command_connect is called when we receive a amf0 command message with the name "connect"
    /// We then handle the connect message
    ...
    /// on_command_create_stream is called when we receive a amf0 command message with the name "createStream"
    /// We then handle the createStream message
    /// This is called when the client wants to create a stream
    ...
    async fn on_command_create_stream(
        ...
        NetConnection::write_create_stream_response(
            &self.chunk_encoder,
            &mut writer,
            transaction_id,
            1.0,
    ...
    /// Delete stream is basically a way to tell the server that you are done publishing or playing a stream.
    /// The server will then remove the stream from its list of streams.
    ...
    /// on_command_publish is called when we receive a amf0 command message with the name "publish"
    /// publish commands are used to publish a stream to the server
    /// ie. the user wants to start streaming to the server
    ...
    /// write_data is a helper function to write data to the underlying connection.
    /// If the data is empty, it will not write anything.
