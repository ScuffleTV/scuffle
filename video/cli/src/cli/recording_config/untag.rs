...
pub struct Untag {
	/// The ids of the recording configs to untag
...
impl Invokable for Untag {
	async fn invoke(&self, invoker: &mut Invoker, _: &Cli) -> anyhow::Result<()> {
		let resp = invoker
			.invoke(pb::scuffle::video::v1::RecordingConfigUntagRequest {
...
impl From<(Ulid, pb::scuffle::video::v1::RecordingConfigUntagResponse)> for TagResponse {
	fn from((id, resp): (Ulid, pb::scuffle::video::v1::RecordingConfigUntagResponse)) -> Self {
		Self {
			id,
