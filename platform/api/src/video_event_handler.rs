...
use pb::scuffle::video::v1::{EventsFetchRequest, types::{event, Event}};
        ...
        if let Some(Event {
            ...
            match evt {
                ...
                event::room::Event::Disconnected(..) => {
                    sqlx::query("UPDATE users SET channel_live_viewer_count = NULL, channel_live_viewer_count_updated_at = NOW() WHERE channel_room_id = $1")
                        .bind(Ulid::from(room_id.into_ulid()))
                        .execute(global.db().as_ref())
                        .await?;
                }
