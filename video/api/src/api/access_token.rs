...
use pb::{
    ...
    scuffle::video::v1::{
        ...
        AccessTokenCreateRequest, AccessTokenCreateResponse, AccessTokenDeleteRequest,
        AccessTokenDeleteResponse, AccessTokenGetRequest, AccessTokenGetResponse,
        AccessTokenTagRequest, AccessTokenTagResponse, AccessTokenUntagRequest,
...
impl<G: ApiGlobal> AccessTokenServiceTrait for AccessTokenServer<G> {
    async fn get(
    ...
    ) -> Result<Response<AccessTokenGetResponse>> {
        let global = get_global(&self.global)?;

        let access_token =
            validate_auth_request(&global, &request, (Resource::AccessToken, Permission::Read))
                .await?;
        ...
        Ok(Response::new(AccessTokenGetResponse {
            access_tokens: results
                .into_iter()
                .map(|access_token| access_token.to_proto())
                .collect(),
        }))
    }
    ...
    ) -> Result<Response<AccessTokenCreateResponse>> {
        let global = get_global(&self.global)?;
    ...
    async fn delete(
    ...
    ) -> Result<Response<AccessTokenDeleteResponse>> {
        let global = get_global(&self.global)?;
        ...
        if request.get_ref().ids.len() > 100 {
            return Err(Status::invalid_argument(
                "too many ids provided for delete: max 100",
            ));
        }

        if request.get_ref().ids.is_empty() {
            return Err(Status::invalid_argument(
                "no ids provided for delete, minimum 1",
            ));
        }
        ...
        if ids.contains(&access_token.id.0) {
            return Err(Status::invalid_argument(
                "cannot delete your own access token",
            ));
        }
        ...
        // We need to make sure we have all the permissions of every token we try to delete.
            ...
            access_token
                ...
                .map_err(|_| {
                    Status::permission_denied(
                        "cannot delete access token with more permissions than you",
                    )
                })?;
        }
        ...
        Ok(Response::new(AccessTokenDeleteResponse {
            ids: tokens_to_delete
                .into_iter()
                .map(|token| token.id.0.into())
                .collect(),
        }))
    }
    ...
    async fn tag(
        &self,
        request: Request<AccessTokenTagRequest>,
    ) -> Result<Response<AccessTokenTagResponse>> {
        let global = get_global(&self.global)?;
    ...
    async fn untag(
        &self,
        request: Request<AccessTokenUntagRequest>,
    ) -> Result<Response<AccessTokenUntagResponse>> {
        let global = get_global(&self.global)?;
