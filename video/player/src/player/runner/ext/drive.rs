use crate::player::{
    ...
    events,
    inner::{PlayerState, VideoTarget},
    runner::{
        source_buffer,
        track::TrackResult,
        utils::{make_media_source_holder, MediaSourceEvent},
        Runner,
    },
    PlayerResult,
};
...
impl Runner {
    async fn drive_audio_track(&mut self, idx: usize) {
        ...
        loop {
            ...
            // We only check if the audio track is finished, because if one is finished, they all are.
            if audio_track.finished() && !self.flags.is_finished {
                ...
                events::dispatch!(self
                    ...
                    .borrow_mut()
                    .events
                    .emit(events::UserEvent::Finished));
            ...
            match audio_track.drive(&self.inner, self.timings.current_player_time) {
                Err(err) => {
                    ...
                    events::dispatch!(self
                        ...
                        .borrow_mut()
                        .events
                        .emit(events::UserEvent::Error(err)));
                ...
                Ok(Some(TrackResult::Media {
                    ...
                    if self.active_audio_track_idx != idx {
                        tracing::trace!(
                            start_time,
                            end_time,
                            "adding audio data to temporary buffer"
                        );
                        self.temporary_audio_buffer
                            .push((data, decode_time, duration));
                        continue;
                        ...
                        if start_time >= next_start {
                            ...
                            tracing::debug!(
                                "removing audio track {idx} from {start_time} to {next_start}"
                            );
                            self.source_buffers
                                .as_mut()
                                .unwrap()
                            ...
                            tracing::trace!(
                                "switching to audio track {idx}",
                                idx = self.next_audio_track_idx.unwrap()
                            );
                            ...
                            for (data, decode_time, duration) in
                                self.temporary_audio_buffer.drain(..)
                            {
                                tracing::trace!(
                                    decode_time,
                                    duration,
                                    "appending audio from temporary buffer"
                                );
                                self.source_buffers
                                    .as_mut()
                                    .unwrap()
                                ...
                                if let Some(video_factory) = self.video_factory.as_mut() {
                                    ...
                                    tracing::trace!(
                                        decode_time,
                                        duration,
                                        "appending video from video factory"
                                    );
                                    self.source_buffers
                                        .as_mut()
                                        .unwrap()
                        ...
                        if let Some(stop_at) = video_factory.will_stop_at() {
                            if self.next_video_track_idx.is_none() {
                                ...
                                self.active_video_track_idx =
                                    Some(self.next_video_track_idx.take().unwrap());
                ...
                Ok(Some(TrackResult::Discontinuity(start, end))) => {
                    if self.inner.borrow().interface_settings.realtime_mode {
                        tracing::error!(
                            "audio discontinuity from {start} to {end}, seeking to {end}"
                        );
        ...
        loop {
            ...
            match video_track.drive(&self.inner, self.timings.current_player_time) {
                ...
                Ok(Some(TrackResult::Media {
                    ...
                    if self.active_video_track_idx != Some(idx) {
                        tracing::trace!(
                            start_time,
                            end_time,
                            "adding video data to temporary buffer"
                        );
                        ...
                        if start_time >= next_start {
                            ...
                            tracing::debug!(
                                "removing video track {idx} from {start_time} to {next_start}"
                            );
                            self.source_buffers
                                .as_mut()
                                .unwrap()
                            ...
                            tracing::trace!(
                                "switching to video track {idx}",
                                idx = self.next_video_track_idx.unwrap()
                            );
                            self.active_video_track_idx =
                                Some(self.next_video_track_idx.take().unwrap());
                            self.video_tracks[self.active_video_track_idx.unwrap()].start();
                ...
                Ok(Some(TrackResult::Discontinuity(start, end))) => {
                    if self.inner.borrow().interface_settings.realtime_mode {
                        tracing::error!(
                            "video discontinuity from {start} to {end}, seeking to {end}"
                        );
                        self.element.set_current_time(end + 0.1);
                        self.timings.current_player_time = end + 0.1;
                    } else if !self.element.paused() {
    ...
    pub async fn drive_running(&mut self, now: f64) {
        ...
        if self.flags.is_finished
            && self.mediasource().unwrap().duration() - self.timings.current_player_time < 0.25
        {
            return;
        }
        ...
        let (mut start_time, mut end_time) =
            self.audio_tracks[self.active_audio_track_idx].seekable_range();

        if let Some(video_track_idx) = self.active_video_track_idx {
            self.drive_video_track(video_track_idx).await;

            let (video_start_time, video_end_time) =
                self.video_tracks[video_track_idx].seekable_range();
    ...
    pub async fn drive_init(&mut self) {
        ...
        events::dispatch!(self
            ...
            .borrow_mut()
            .events
            .emit(events::UserEvent::LoadStart));
        ...
        if let Err(err) = match target {
            ...
            self.player_url =
                Some(web_sys::Url::create_object_url_with_source(&mediasource).unwrap());
            ...
            match mediasource.events().recv().await {
                ...
                None | Some(MediaSourceEvent::SourceClose) => {
                    events::dispatch!(self.inner.borrow_mut().events.emit(
                        events::UserEvent::Error(EventError::new(
                            ErrorCode::Other,
            ...
            self.source_buffers = Some(source_buffer::SourceBuffers::new(
                self.mediasource().unwrap(),
            ));
