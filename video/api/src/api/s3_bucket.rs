...
impl<G: ApiGlobal> S3BucketServiceTrait for S3BucketServer<G> {
	async fn get(&self, _request: Request<S3BucketGetRequest>) -> Result<Response<S3BucketGetResponse>> {
		todo!("TODO: implement S3Bucket::get")
	}

	async fn create(&self, _request: Request<S3BucketCreateRequest>) -> Result<Response<S3BucketCreateResponse>> {
		todo!("TODO: implement S3Bucket::create")
	}

	async fn modify(&self, _request: Request<S3BucketModifyRequest>) -> Result<Response<S3BucketModifyResponse>> {
		todo!("TODO: implement S3Bucket::modify")
	}

	async fn delete(&self, _request: Request<S3BucketDeleteRequest>) -> Result<Response<S3BucketDeleteResponse>> {
		todo!("TODO: implement S3Bucket::delete")
	}

	async fn tag(&self, request: Request<S3BucketTagRequest>) -> Result<Response<S3BucketTagResponse>> {
		let global = get_global(&self.global)?;

		let access_token = validate_auth_request(&global, &request, (Resource::S3Bucket, Permission::Modify)).await?;
	...
	async fn untag(&self, request: Request<S3BucketUntagRequest>) -> Result<Response<S3BucketUntagResponse>> {
		let global = get_global(&self.global)?;
