...
use super::{layer::SpanHolder, node::SpanNode};
        ...
        if let Err(err) = fut.await {
            ...
            if internal.config.metrics {
                let reason = match err {
                    TraceError::ExportTimedOut(_) => {
                        opentelementry::SpanDroppedReason::ExportTimeout
                    }
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            for chunk in self
                ...
                .into_iter()
            {
                let Ok(pending_permit) =
                    self.internal.pending_semaphore.clone().try_acquire_owned()
                else {
                    drop_pending = true;
                    break;
                };
            ...
            #[cfg(feature = "metrics")]
            if self.internal.config.metrics {
                opentelementry::spans_dropped(
                    opentelementry::SpanDroppedReason::PendingExportBackpressure,
                )
