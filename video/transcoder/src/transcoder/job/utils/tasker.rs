...
pub fn delete_media_generator<G: TranscoderGlobal>(key: String) -> impl Fn(Arc<G>) -> TaskFuture + Send + Sync + 'static {
	move |global: Arc<G>| {
		let global = global.clone();
		let key = key.clone();
		async move {
...
pub fn upload_segment_generator<G: TranscoderGlobal>(
	...
	move |global| {
		...
		Box::pin(async move {
			...
			.rows_affected() != 1
			{
				return Err(TaskError::Custom(anyhow::anyhow!("Failed to update recording rendition")));
			}
	...
	move |global| {
		...
		Box::pin(async move {
			...
			state
				...
				.put_object_stream_with_content_type(
					&mut cursor,
					&video_common::keys::s3_thumbnail(state.organization_id, state.recording_id, partial.idx, partial.id),
					"image/jpg",
				)
				.await?;
