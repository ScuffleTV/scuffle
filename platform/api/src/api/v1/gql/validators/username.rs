...
impl CustomValidator<String> for UsernameValidator {
	fn check(&self, value: &String) -> Result<(), InputValueError<String>> {
		if value.len() < 3 {
			return Err(InputValueError::custom("Username must be at least 3 characters long"));
		}

		if value.len() > 20 {
			return Err(InputValueError::custom("Username must be at most 20 characters long"));
		}
