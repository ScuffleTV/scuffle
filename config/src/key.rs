...
impl KeyPath {
	...
	/// Adds a new [`KeyPathSegment::Map`](KeyPathSegment::Map) to this key
	/// path.
	...
	/// Adds a new [`KeyPathSegment::Seq`](KeyPathSegment::Seq) to this key
	/// path.
	...
	/// Adds a new [`KeyPathSegment::Map`](KeyPathSegment::Map) containing a
	/// [`Value::String`](Value::String) to this key path.
...
pub struct KeyGraphBuilder<C: std::any::Any> {
	...
	// We use a NonNull<C> here to make sure the compiler enforces that KeyGraphBuilder is not
	// Send, since its a thread_local type
...
impl<C: std::any::Any> KeyGraphBuilder<C> {
	...
	pub fn build(self, graph: KeyGraph) -> Arc<KeyGraph> {
		...
		// no other thread can access it. This function also takes ownership of the
		// graph builder. Which means that no other thread can access the graph builder,
		// and also this function will not be called again. This means that the graph
		// will not be mutated again. If this function is not called and the builder is
		// dropped, then the Arc<KeyGraph> will be dropped, Therefore the weak pointer
		// will be empty and the graph will be rebuilt when the builder is created
		// again.
		unsafe {
			...
			if let Some((graph, building)) = mg.get(&ty) {
				if let Some(graph) = graph.upgrade() {
					return KeyGraphBuilder {
						...
						// 1) Allows for graph memoization (we dont have to rebuild a graph if we already have one for this
						//    type) (if building is false)
						// 2) Allows for recursive types (if building is true, we have not finished building the graph yet
						//    but a type has requested it, so we return a reference to the graph)
						mode: if *building { BuilderMode::Ref } else { BuilderMode::Owned },
...
/// Function used to transform a value from whatever it is to the type of the
/// key.
