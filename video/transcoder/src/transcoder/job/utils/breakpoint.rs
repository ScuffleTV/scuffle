...
impl<'a> BreakpointState<'a> {
    ...
    pub fn process_segment_break(
        &mut self,
        target_segment_duration: f64,
        max_part_duration: f64,
    ) -> bool {
        if self.is_segment_break(target_segment_duration)
            && self.add_segment_break(max_part_duration)
        {
            true
    ...
    pub fn process_part_break(
        ...
        if self.is_part_break(target_part_duration) && self.add_part_break() {
            true
        } else if let Some((idx, part_duration, segment_duration)) =
            self.check_potential_part_break(max_part_duration)
        {
            self.idx = idx;
    ...
    fn is_segment_break(&self, target_segment_duration: f64) -> bool {
        self.current_sample()
            .map(|s| s.keyframe)
            .unwrap_or_default()
            && self.segment_time() >= target_segment_duration
    }
    ...
    fn merge_last_breakpoint(&mut self, max_part_duration: f64) {
        if let Some((_, breaktype)) = self.break_points.last() {
            if *breaktype == BreakType::Part
                && (self.durations.last_part + self.durations.part - self.current_sample_duration())
                    as f64
                    / self.timescale as f64
                    <= max_part_duration
            {
                // If the last break point was a part break, and the last part duration + this part duration is less than the max part duration, we remove the last break point.
                // This is because we want to merge the last part with this part.
    ...
    fn add_part_break(&mut self) -> bool {
        if self.is_perfect_part_break() {
            ...
            self.potential_breaks.part =
                Some((self.idx, self.durations.part, self.durations.segment));
            false
        } else {
            false
        }
