...
use crate::player::{
    ...
    events::{self, VariantEvent},
    inner::PlayerState,
    runner::{track::TrackState, Runner},
    variant, PlayerResult,
};
...
impl Runner {
    fn handle_resp(&mut self, mut resp: SessionPlaylist) -> PlayerResult<()> {
        ...
        if resp.audio_tracks.is_empty() {
            return Err(EventError::new(
                ErrorCode::Other,
                "no supported audio tracks".into(),
                true,
            ));
        }
        ...
        for (audio_id, audio) in resp.audio_tracks.iter().enumerate() {
            ...
            for (video_id, video) in self.video_tracks.iter().enumerate() {
                variants.push(variant::Variant::new(
                    (audio_id, audio),
                    Some((video_id, video.track())),
                ));
            }
        }

        // todo this is wrong, its not always the highest bitrate = highest quality
        variants.sort_unstable_by_key(|s| {
            s.audio_track.bitrate
                + s.video_track
                    .as_ref()
                    .map(|v| v.bitrate)
                    .unwrap_or_default()
        ...
        events::dispatch!(self
            ...
            .borrow_mut()
            .events
            .emit(events::UserEvent::ManifestLoaded));

        events::dispatch!(self
            .inner
            .borrow_mut()
            .events
            .emit(events::UserEvent::Variant(VariantEvent {
                automatic: None,
                variant_id,
    ...
    pub async fn load_room(&mut self, room_id: Ulid) -> PlayerResult<()> {
        ...
        let mut room_req = self.inner.borrow().client.get_room(
            room_id,
            self.inner.borrow().interface_settings.token.as_deref(),
        );
    ...
    pub async fn load_recording(&mut self, recording_id: Ulid) -> PlayerResult<()> {
        ...
        let mut recording_req = self.inner.borrow().client.get_recording(
            recording_id,
            self.inner.borrow().interface_settings.token.as_deref(),
        );
