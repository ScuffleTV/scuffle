...
use crate::{transform_from_graph, Config, ConfigError, ConfigErrorType, KeyGraph, KeyPath, KeyPathSegment, Result, Value};
		...
		match &*graph {
			...
			KeyGraph::Struct(values) => {
				let key = key.ok_or_else(|| {
					ConfigError::new(ConfigErrorType::ValidationError(format!("expected map, found {:?}", graph)))
				})?;
				...
				return Ok(Some((Arc::new(KeyGraph::String), Arc::new(value.graph().clone()))));
			}
...
pub fn graph_to_seq(mut graph: Arc<KeyGraph>, mut visited: HashSet<&'static str>) -> Result<Option<Arc<KeyGraph>>> {
	loop {
		match &*graph {
...
pub fn value_to_map<'a>(path: &KeyPath, mut value: &'a Value) -> Result<Option<&'a BTreeMap<Value, Value>>> {
	loop {
		match value {
			...
			_ => {
				return Err(ConfigError::new(ConfigErrorType::SubkeyOnNonMap(value.clone())).with_path(path.clone()));
			}
		}
	}
}
...
pub fn value_to_seq<'a>(path: &KeyPath, mut value: &'a Value) -> Result<Option<&'a Vec<Value>>> {
	loop {
		match value {
			...
			_ => {
				return Err(ConfigError::new(ConfigErrorType::SubIndexOnNonSeq(value.clone())).with_path(path.clone()));
			}
		}
	}
}
	...
	for segment in path {
		match segment {
			KeyPathSegment::Map { key } => {
				...
				let Some((key_graph, value_graph)) = graph_to_map(graph, key_str.as_deref(), HashSet::new())? else {
					return Ok(None);
				};
