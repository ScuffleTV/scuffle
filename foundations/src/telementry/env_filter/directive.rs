...
impl<T: Match + Ord> DirectiveSet<T> {
	...
	pub(crate) fn directives_for<'a>(&'a self, metadata: &'a Metadata<'a>) -> impl Iterator<Item = &'a T> + 'a {
	...
	#[cfg(test)]
	pub(crate) fn into_vec(self) -> FilterVec<T> {
		self.directives
	}
}
...
impl DirectiveSet<StaticDirective> {
	...
	pub(crate) fn directives_for_target<'a>(&'a self, target: &'a str) -> impl Iterator<Item = &'a StaticDirective> + 'a {
		self.directives().filter(move |d| d.cares_about_target(target))
	}
}
...
impl StaticDirective {
	pub(crate) fn new(target: Option<String>, field_names: Vec<String>, level: LevelFilter) -> Self {
		Self {
			target,
			field_names,
			level,
...
impl FromStr for StaticDirective {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		let part0 = split.next().ok_or_else(|| ParseError::msg("string must not be empty"))?;
		...
		if let Some(part1) = split.next() {
			if split.next().is_some() {
				return Err(ParseError::msg("too many '=' in filter directive, expected 0 or 1"));
			}
...
impl From<ParseLevelFilterError> for ParseError {
	fn from(l: ParseLevelFilterError) -> Self {
		Self {
