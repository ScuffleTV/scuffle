...
use crate::tracing_wasm::{self, WASMLayerConfig};
...
const DESTROYED_ERROR: &str =
    "player has been destroyed, you must create a new instance to continue.";
...
impl Player {
    ...
    pub fn load_room(&self, room_id: String, token: Option<String>) -> JsResult<()> {
        ...
        let Ok(room_id) = room_id.parse() else {
            return Err(JsValue::from_str(
                "invalid room id, room ids must be valid ULIDs",
            ));
        };
    ...
    pub fn load_recording(&self, recording_id: String, token: Option<String>) -> JsResult<()> {
        ...
        let Ok(recording_id) = recording_id.parse() else {
            return Err(JsValue::from_str(
                "invalid recording id, recording ids must be valid ULIDs",
            ));
        };

        self.inner.borrow_mut().interface_settings.state = PlayerState::Initialized;
        self.inner.borrow_mut().interface_settings.target =
            Some(inner::VideoTarget::Recording(recording_id));
    ...
    pub fn on(&self, event: String, f: JsValue) -> JsResult<()> {
        ...
        let Ok(ty) = event.parse() else {
            return Err(JsValue::from_str(
                format!("invalid event type: {event}").as_str(),
            ));
        };
    ...
    pub fn to_realtime(&self) -> JsResult<()> {
        ...
        if self.inner.borrow().interface_settings.state != PlayerState::Running {
            return Err(JsValue::from_str(
                "realtime mode can only be set while the player is running",
            ));
        }
    ...
    pub fn set_low_latency(&self, low_latency: bool) -> JsResult<()> {
        ...
        self.inner
            ...
            .interface_settings
            .player_settings
            .enable_low_latency = low_latency;

        events::dispatch!(self
            .inner
            .borrow_mut()
            .events
            .emit(events::UserEvent::LowLatency));
    ...
    pub fn set_variant_id(&self, variant_id: i32) -> JsResult<()> {
        ...
        if variant_id == -1 {
            ...
            self.inner.borrow_mut().interface_settings.next_variant_id =
                Some(inner::NextVariant::Force(variant_id as u32));
        }
    ...
    pub fn set_next_variant_id(&self, variant_id: i32) -> JsResult<()> {
        ...
        if variant_id == -1 {
            ...
            self.set_abr_enabled(false)?;
            self.inner.borrow_mut().interface_settings.next_variant_id =
                Some(inner::NextVariant::Switch(variant_id as u32));
        }
    ...
    pub fn seek_thumbnail(&self, time: f64) -> JsResult<Option<ThumbnailRange>> {
        ...
        if inner.runner_settings.thumbnails.is_empty()
            || inner.runner_settings.thumbnail_prefix.is_none()
        {
            return Ok(None);
        }
        ...
        match r {
            ...
            Err(idx) => {
                ...
                let (closest, _) = [before, first, second].iter().enumerate().fold(
                    (0, time),
                    |(idx, min), (i, v)| {
                        let Some(v) = *v else {
                            return (idx, min);
                ...
                let thumbnail =
                    &inner.runner_settings.thumbnails[(idx + closest).saturating_sub(1)];
                Ok(Some(ThumbnailRange {
                    url: format!("{prefix}/{}", thumbnail.id),
                    time: thumbnail.start_time,
                }))
