...
pub struct ChunkMessageHeader {
    pub timestamp: u32, // 3 bytes (when writing the header, if the timestamp is >= 0xFFFFFF, write 0xFFFFFF)
...
impl ChunkMessageHeader {
    ...
    /// This means that the timestamp is extended and is written in the extended timestamp field.
...
/// This is the maximum chunk size we will accept. If the peer requests a chunk size bigger than this, we will close the connection.
