...
fn test_decoder_error_display() {
    ...
    assert_eq!(
        format!("{}", error),
        "timestamp overflow: timestamp: 100, delta: 200"
    );
}
...
fn test_decoder_chunk_type0_double_sized() {
    ...
    // We just feed the same data again in this test to see if the Unpacker merges the chunks
    // Which it should do
    unpacker.extend_data(&chunk);
...
fn test_decoder_chunk_mutli_streams() {
    ...
    // Even though we wrote chunk 3 first, chunk 4 should be read first since it's a different stream
    let chunk = unpacker.read_chunk().expect("read chunk").expect("chunk");
