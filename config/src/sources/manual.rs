...
use super::utils;
use crate::{Config, ConfigError, ConfigErrorType, ErrorSource, KeyPath, KeyPathSegment, Result, Source, Value};
			...
			KeyPathSegment::Seq { index } => {
				if index == 0 {
					...
					return Err(ConfigError::new(ConfigErrorType::ValidationError(
						"indices other than 0 not supported when setting values with manual source".to_string(),
					)));
				}
			}
		}
