...
pub use layer::{
    RatelimitSampler, SampleFunction, SampleResult, Sampler, ShouldSample, SpanObserver,
    SpanObserverLayer,
};
...
pub fn complex_rate_sampler(
    ...
    Sampler::function(move |node| {
        ...
        let rate_to_use = if node.contains_error() {
            error_rate.unwrap_or(if node.is_child() {
                tail_rate
            } else {
                head_rate
            })
        ...
        if node.is_child() {
            ...
            match Sampler::TraceIdRatio(rate_to_use).should_sample(node) {
                SampleResult::Sample
                    if (!sample_on_error || !node.contains_error()) && tail_rate != head_rate =>
                {
                    let should_sample_children =
                        Sampler::TraceIdRatio(tail_rate).should_sample(node);
