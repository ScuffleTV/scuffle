...
use super::super::directive::{DirectiveSet, Match};
pub(crate) use super::super::directive::{FilterVec, ParseError, StaticDirective};
use super::super::env::{field, FieldMap};
...
impl Directive {
	...
	pub(super) fn to_static(&self) -> Option<StaticDirective> {
		...
		Some(StaticDirective::new(self.target.clone(), field_names, self.level))
	}
	...
	pub(crate) fn field_matcher(&self, meta: &Metadata<'_>) -> Option<field::CallsiteMatch> {
		...
		let fields = self
			.fields
			.iter()
			.filter_map(|field::Match { ref name, ref value }| {
	...
	pub(super) fn make_tables(directives: impl IntoIterator<Item = Directive>) -> (Dynamics, Statics) {
		// TODO(eliza): this could be made more efficient...
		let (dyns, stats): (Vec<Directive>, Vec<Directive>) = directives.into_iter().partition(Directive::is_dynamic);
		let statics = stats
			.into_iter()
	...
	pub(super) fn deregexify(&mut self) {
		for field in &mut self.fields {
			field.value = match field.value.take() {
				Some(field::ValueMatch::Pat(pat)) => Some(field::ValueMatch::Debug(pat.into_debug_match())),
				x => x,
			}
		}
	}
	...
	pub(super) fn parse(from: &str, regex: bool) -> Result<Self, ParseError> {
		...
		if let Some(level) = caps.name("global_level").and_then(|s| s.as_str().parse().ok()) {
			return Ok(Directive {
				level,
				..Default::default()
			});
...
impl Dynamics {
	...
	pub(crate) fn has_value_filters(&self) -> bool {
		self.directives().any(|d| d.fields.iter().any(|f| f.value.is_some()))
	}
}
...
mod test {
	...
	fn directive_ordering_by_target_len() {
		...
		// instead.
		let mut dirs = expect_parse("foo::bar=debug,foo::bar::baz=trace,foo=info,a_really_long_name_with_no_colons=warn");
		dirs.sort_unstable();

		let expected = vec!["a_really_long_name_with_no_colons", "foo::bar::baz", "foo::bar", "foo"];
		let sorted = dirs.iter().map(|d| d.target.as_ref().unwrap()).collect::<Vec<_>>();
	...
	fn directive_ordering_uses_lexicographic_when_equal() {
		...
		let sorted = dirs
			.iter()
			.map(|d| (d.target.as_ref().unwrap().as_ref(), d.in_span.as_ref().map(String::as_ref)))
			.collect::<Vec<_>>();
