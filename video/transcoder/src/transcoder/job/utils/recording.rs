...
use pb::{
    ext::UlidExt,
    scuffle::video::{
        internal::live_rendition_manifest::recording_data::RecordingThumbnail,
        v1::types::{AudioConfig, RecordingConfig, VideoConfig},
    },
};
use prost::Message;
use s3::Region;
...
impl Recording {
    ...
    pub async fn new(
            ...
            s3::Bucket::new(
                &s3_bucket.name,
                {
                    let region = s3_bucket
                        ...
                        .ok_or_else(|| {
                            anyhow::anyhow!("Invalid S3 region: {:?}", s3_bucket.region)
                        })?;
                    match region {
        ...
        let mut qb = sqlx::QueryBuilder::new(
            "INSERT INTO recording_renditions (recording_id, rendition, config)",
        );
        ...
        qb.push_values(
            video_outputs.chain(audio_outputs),
            |mut b, (rendition, config)| {
        ...
        Ok(Self {
            ...
            renditions: recording_config
                ...
                .iter()
                .map(|r| {
                    Rendition::from(pb::scuffle::video::v1::types::Rendition::try_from(*r).unwrap())
                })
                .collect(),
