...
fn metric_function(
	...
	let make_metric = {
		let make_metric = if let Some(builder) = item.attrs.options.builder.as_ref().or(options.builder.as_ref()) {
...
pub fn metrics(args: proc_macro::TokenStream, input: proc_macro::TokenStream) -> syn::Result<proc_macro2::TokenStream> {
