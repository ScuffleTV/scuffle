...
use crate::player::{
    api::{ApiMediaClient, ApiSessionClient, Json, RenditionSettings, ScufflePart},
    errors::{ErrorCode, EventError, EventErrorExtFetch},
    fetch::{FetchError, FetchRequest},
...
use self::{
    regions::{SegmentRangeResult, SegmentRegions, TimeRegion, TimeRegions},
    requests::{RequestQueue, TrackRequest},
};
...
pub enum TrackResult {
    ...
    /// The first value is the bytes of the media, the second value is the start time of the media, the third value is the end time of the media
    Media {
        data: Bytes,
        start_time: f64,
        end_time: f64,
    ...
    /// A discontinuity has occurred, the first value is the end time of the previous segment, the second value is the start time of the next segment
...
impl<T> TrackState<T> {
    ...
    pub fn drive(
            ...
            || (!low_latency_enabled && self.last_requested_part_idx.is_some())
        {
            tracing::debug!(
                ...
                "resetting requests, due to low latency being toggled"
            );
            self.requests.clear();
            self.requested_regions.clear();
            self.last_requested_part_idx = None;
        ...
        if !dvr_enabled && !self.segment_regions.is_empty() {
            tracing::debug!(
                name = self.track.name,
                "resetting segment regions, due to dvr being disabled"
            );
        ...
        if self.manifest.is_none() || self.init_segment.is_none() {
            tracing::trace!(
                name = self.track.name,
                "manifest or init segment is none, returning"
            );
    ...
    pub fn drive_media(
        ...
        if self.start_from.is_some() {
            tracing::trace!(
                name = self.track.name,
                "starting from is some, drive media cannot be run"
            );
            return Ok(None);
        }
                ...
                if low_latency_enabled {
                    if let Some(part_idx) = self.last_requested_part_idx {
                        if let Some((id, _)) = manifest.part(part_idx + 1) {
                            ...
                            tracing::trace!(
                                name = self.track.name,
                                "requesting part: {part_idx}",
                                part_idx = part_idx + 1
                            );
                            self.last_requested_part_idx = Some(part_idx + 1);
                        }
                    } else if let Some(segment) = manifest
                        .segments
                        .get(manifest.segments.len().saturating_sub(1))
                    {
                    ...
                    .segments
                    .get(manifest.segments.len().saturating_sub(2))
                {
                    if let Some(id) = segment.id.as_deref() {
                        self.requests.push(TrackRequest::new(
                            media_client.get_mp4(id),
            ...
            SegmentRangeResult::Range(s) => {
                ...
                if let Some(region) = self.requested_regions.get(s.start) {
                    if region.end
                        ...
                        < self.player_time
                    {
                        if let SegmentRangeResult::Range(next_segment) =
                            self.segment_regions.get(region.end)
                        {
                            if next_segment.idx != s.idx {
                                self.requests.push(TrackRequest::new(
                                    FetchRequest::new(
                                        "GET",
                                        dvr_prefix.join(&next_segment.dvr_tag).unwrap(),
                                    ),
                                    requests::RequestIndex::Segment {
                                ...
                                self.requested_regions
                                    .add(next_segment.start, next_segment.end);
                            }
                        }
                    }
                } else {
    ...
    pub fn drive_manifest(
                ...
                .unwrap_or_default()
        {
            tracing::debug!(
                name = self.track.name,
                "manifest is stale, resetting manifest"
            );
            self.manifest = None;
            self.manifest_request = None;
            self.requests.clear();
        ...
        if let Some(request) = self.manifest_request.as_mut() {
            request
                ...
                .map_err(|mut err| {
                    ...
                    self.manifest_refresh = ManifestRefresh::Time(
                        now() + (self.manifest_req_error_count - 1) as f64 * ERROR_BACKOFF,
                    );
            ...
            match self.manifest_refresh {
                ...
                ManifestRefresh::IPart(idx) => {
                    ...
                    //     tracing::debug!(
                    //         name = self.track.name,
                    //         "last fetched part idx is greater than requested part idx, requesting manifest"
                    //     );
                    self.manifest_request = Some(ManifestRequest {
    ...
    pub fn drive_init(&mut self, inner: &PlayerInnerHolder) -> PlayerResult<()> {
        ...
        if self.next_init_req_time > now() {
            tracing::trace!(
                name = self.track.name,
                "next init req time is greater than now, returning"
            );
        ...
        if let Some(request) = self.init_request.as_mut() {
                ...
                .unwrap_or_default()
            {
                let Some(init) = self
                    ...
                    .map_err(|mut err| {
                        ...
                        self.next_init_req_time =
                            now() + (self.init_req_error_count - 1) as f64 * ERROR_BACKOFF;
                    ...
                    return Err(EventError::new(
                        ErrorCode::Other,
                        "failed to fetch init segment".into(),
                        true,
                    ));
                };
                ...
                match mp4::DynBox::demux(&mut cursor) {
                    ...
                    Ok(mp4) => {
                        tracing::error!(
                            "invalid init segment: expected ftyp box got {}",
                            mp4.name()
                        );
                        return Err(EventError::new(
                            ErrorCode::Decode,
                ...
                let moov = match mp4::DynBox::demux(&mut cursor) {
                    ...
                    Ok(mp4) => {
                        tracing::error!(
                            "invalid init segment: expected moov box got {}",
                            mp4.name()
                        );
                        return Err(EventError::new(
                            ErrorCode::Decode,
            ...
            let req = if manifest.init_dvr {
                ...
                self.media_client
                    .as_ref()
                    .unwrap()
                    .get_mp4(&manifest.init_segment_id)
            };
    ...
    pub fn handle_manifest(
            ...
            match start_from {
                StartFrom::IPart(_) => {
                    let part = manifest
                        ...
                        .unwrap();
                    self.last_requested_part_idx =
                        manifest.part_idx(&part.id).unwrap().0.checked_sub(1);
        ...
        if was_dvr {
            ...
            manifest.thumbnails.iter().for_each(|t| {
                match inner
                    .runner_settings
                    .thumbnails
                    .binary_search_by(|a| a.idx.cmp(&t.idx))
                {
                    Ok(idx) => {
        ...
        if manifest.finished {
            ...
            if let Some(active_segment) = manifest.segments.iter().find(|s| s.id.is_some()) {
                self.segment_regions
                    .active_range(active_segment.start_time.unwrap());
            }
        }
