...
impl Match {
	...
	pub(crate) fn parse(s: &str, regex: bool) -> Result<Self, Box<dyn Error + Send + Sync>> {
		...
		let name = parts
			.next()
			.ok_or_else(|| BadName { name: "".to_string() })?
...
impl ValueMatch {
	...
	fn parse_regex(s: &str) -> Result<Self, matchers::Error> {
		s.parse::<bool>()
			...
			.or_else(|_| s.parse::<MatchPattern>().map(|p| ValueMatch::Pat(Box::new(p))))
	}
...
impl MatchPattern {
	...
	pub(super) fn into_debug_match(self) -> MatchDebug {
		MatchDebug { pattern: self.pattern }
	}
}
...
impl MatchDebug {
	...
	fn debug_matches(&self, d: &impl fmt::Debug) -> bool {
		...
		let mut matcher = Matcher { pattern: &self.pattern };
...
impl SpanMatch {
	...
	fn is_matched_slow(&self) -> bool {
		let matched = self.fields.values().all(|(_, matched)| matched.load(Acquire));
		if matched {
...
impl<'a> Visit for MatchVisitor<'a> {
	...
	fn record_str(&mut self, field: &Field, value: &str) {
		match self.inner.fields.get(field) {
			...
			Some((ValueMatch::Debug(ref e), ref matched)) if e.debug_matches(&value) => matched.store(true, Release),
			_ => {}
		}
	}
...
mod tests {
	...
	fn debug_struct_match() {
		...
		let matcher = MatchDebug { pattern: pattern.into() };
