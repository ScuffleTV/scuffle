...
use self::{types::DataLoaderInner, utils::new_auto_loader};
...
impl<L: Loader<S> + Default, S: Send + Sync + Default + BuildHasher + 'static> Default
    for DataLoader<L, S>
{
    fn default() -> Self {
        Self::new(L::default())
...
impl<L: Loader<S>, S: Send + Sync + Default + BuildHasher + 'static> DataLoader<L, S> {
    ...
    #[inline(always)]
    pub async fn load_with_cache<C: Cache<L, S>>(
        &self,
        cache: C,
        key: L::Key,
    ) -> Result<Option<L::Value>, L::Error> {
        Ok(self
