...
use crate::telementry::opentelemetry::{
    complex_rate_sampler, BatchExporter, Sampler, SpanObserver,
};
...
pub enum OpentelemetrySettingsSampler {
    ...
    RatioSimple(f64),
    /// Sample spans based on a rate, with the ability to set a different rate for root spans.
    /// This is useful because you can always sample root spans and then on some rate cull the tail.
    /// In production, you might want to sample all root spans and then sample tail spans at a lower rate.
    RatioComplex {
...
pub struct ServerSettings {
    ...
    pub bind: SocketAddr,
    /// The path to the pprof heap endpoint. If `None`, the endpoint is disabled.
                ...
                crate::telementry::opentelemetry::layer(
                    SpanObserver {
                        ...
                        sampler: match settings.opentelemetry.sampler {
                            ...
                            OpentelemetrySettingsSampler::RatioSimple(rate) => {
                    ...
                    BatchExporter {
                        ...
                        resource: {
                            ...
                            if !settings.opentelemetry.labels.contains_key("service.name") {
                                kv.push(opentelemetry::KeyValue::new(
                                    "service.name",
                                    info.metric_name,
                                ));
                            }
                            ...
                            kv.extend(
                                settings.opentelemetry.labels.iter().map(|(k, v)| {
                                    opentelemetry::KeyValue::new(k.clone(), v.clone())
                                }),
                            );
                        ...
                        drop_handler: {
                            const DROPPED_SPANS_ERROR: &str =
                                "opentelementry exporter dropped spans due to backpressure";
                        ...
                        error_handler: {
                            const EXPORTER_ERROR: &str =
                                "opentelementry exporter failed to export spans";

                            match settings.opentelemetry.logging.exporter_errors {
                                OpentelemetrySettingsLoggingLevel::Error => {
                                    Box::new(|err, count| {
                                        tracing::error!(err = %err, count, EXPORTER_ERROR);
                                    })
                                }
                                OpentelemetrySettingsLoggingLevel::Warn => {
                                    Box::new(|err, count| {
                                        tracing::warn!(err = %err, count, EXPORTER_ERROR);
                                    })
                                }
                                OpentelemetrySettingsLoggingLevel::Info => {
                                    Box::new(|err, count| {
                                        tracing::info!(err = %err, count, EXPORTER_ERROR);
                                    })
                                }
                                OpentelemetrySettingsLoggingLevel::Debug => {
                                    Box::new(|err, count| {
                                        tracing::debug!(err = %err, count, EXPORTER_ERROR);
                                    })
                                }
                                OpentelemetrySettingsLoggingLevel::Trace => {
                                    Box::new(|err, count| {
                        ...
                        export_handler: {
                            const EXPORTER_SUCCESS: &str =
                                "opentelementry exporter successfully exported spans";
                    ...
                    },
                    {
                        match settings.opentelemetry.otlp_method {
                            OpentelemetrySettingsExportMethod::Grpc => {
                            ...
                            OpentelemetrySettingsExportMethod::Http => {
                                opentelemetry_otlp::new_exporter()
                                    .http()
                                    .with_endpoint(settings.opentelemetry.otlp_endpoint.clone())
                                    .with_timeout(settings.opentelemetry.otlp_timeout)
        ...
        let logging = if settings.logging.enabled {
            let layer = tracing_subscriber::fmt::layer()
                ...
                .with_timer(match settings.logging.timestamps {
                    LoggingSettingsTimestamps::Local => {
                        TimeFormatter::Local(ChronoLocal::rfc_3339())
                    }
