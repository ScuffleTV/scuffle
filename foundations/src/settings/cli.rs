    ...
    fn load_file(file: &str, optional: bool) -> anyhow::Result<Option<serde_yaml::Value>> {
        let contents = match std::fs::read_to_string(file) {
            ...
            Err(err) => {
                ...
                return Err(err)
                    .with_context(|| format!("Error reading configuration file: {file}"));
            }
        };

        let incoming = serde_yaml::from_str(&contents)
            .with_context(|| format!("Error parsing configuration file: {file}"))?;
    ...
    pub fn parse(mut self) -> anyhow::Result<Matches<S>> {
        ...
        if let Some(file) = args.get_one::<String>(GENERATE_ARG_ID) {
            ...
            std::fs::write(file, settings)
                .with_context(|| format!("Error writing configuration file: {file}"))?;
