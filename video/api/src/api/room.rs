...
impl<G: ApiGlobal> RoomServiceTrait for RoomServer<G> {
	...
	async fn create(&self, _request: Request<RoomCreateRequest>) -> Result<Response<RoomCreateResponse>> {
		todo!("TODO: implement Room::create")
	}

	async fn modify(&self, _request: Request<RoomModifyRequest>) -> Result<Response<RoomModifyResponse>> {
		todo!("TODO: implement Room::modify")
	}

	async fn delete(&self, _request: Request<RoomDeleteRequest>) -> Result<Response<RoomDeleteResponse>> {
		todo!("TODO: implement Room::delete")
	}

	async fn disconnect(&self, _request: Request<RoomDisconnectRequest>) -> Result<Response<RoomDisconnectResponse>> {
		todo!("TODO: implement Room::disconnect")
	}

	async fn reset_key(&self, _request: Request<RoomResetKeyRequest>) -> Result<Response<RoomResetKeyResponse>> {
	...
	async fn tag(&self, request: Request<RoomTagRequest>) -> Result<Response<RoomTagResponse>> {
		...
		let access_token = validate_auth_request(&global, &request, (Resource::Room, Permission::Modify)).await?;
		...
		let updated_tags = add_tag_query(&global, "rooms", &tags.tags, id, Some(access_token.organization_id.into()))
			.await?
	...
	async fn untag(&self, request: Request<RoomUntagRequest>) -> Result<Response<RoomUntagResponse>> {
		let global = get_global(&self.global)?;
