...
impl Cpu {
	...
	pub fn capture(&self, duration: std::time::Duration) -> anyhow::Result<Vec<u8>> {
		let profiler = self.0.clone().build().context("failed to build pprof profiler")?;

		std::thread::sleep(duration);

		let report = profiler.report().build().context("failed to build pprof report")?;
		...
		gz.finish().context("failed to finish compressing pprof profile")
	}
}
