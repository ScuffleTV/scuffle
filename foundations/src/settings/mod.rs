...
impl<S> SettingsParser<S> {
	...
	fn merge_loop(&self, root: serde_yaml::Value, incoming: serde_yaml::Value, merge: MergeDirective) -> serde_yaml::Value {
		match (root, incoming) {
			(serde_yaml::Value::Mapping(mut first_map), serde_yaml::Value::Mapping(second_map)) => {
				for (key, value) in second_map {
					...
					let (key, merge) = match key {
						serde_yaml::Value::Tagged(tagged) => (tagged.value, MergeDirective::from_tag(&tagged.tag)),
			...
			(serde_yaml::Value::Sequence(mut first_seq), serde_yaml::Value::Sequence(second_seq)) => {
	...
	fn handle_tagged(
		...
		if matches!(MergeDirective::from_tag(&tagged.tag), MergeDirective::Unset) {
			serde_yaml::Value::Tagged(Box::new(serde_yaml::value::TaggedValue { tag: tagged.tag, value }))
		} else {
			value
		}
	}
...
pub fn to_yaml_string<T: serde::Serialize + Settings>(settings: &T) -> Result<String, serde_yaml::Error> {
...
pub fn to_yaml_string_with_docs<T: serde::Serialize>(settings: &T, docs: &DocMap) -> Result<String, serde_yaml::Error> {
...
macro_rules! push_docs {
	($result:expr, $docs:expr, $stack:expr, $indent:expr) => {{
		$docs.get($stack).into_iter().flat_map(|s| s.iter()).for_each(|doc| {
...
fn convert_recursive(docs: &DocMap, stack: &mut Vec<CowStr>, value: &Value, result: &mut String, indent: usize) {
	...
	match value {
		...
		_ => {
			...
			// TODO(troy): figure out a way to do sub-docs for scalars so that
			// the format             isnt so janky
