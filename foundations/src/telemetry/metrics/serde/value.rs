...
#[inline]
pub(super) fn serializer(writer: LabelValueEncoder<'_>) -> impl Serializer<Ok = (), Error = super::Error> + '_ {
...
impl Serializer for ValueSerializer<'_> {
	...
	fn serialize_f32(mut self, v: f32) -> Result<Self::Ok, Self::Error> {
		(v as f64).encode(&mut self.writer).map_err(Self::Error::Fmt)?;
	...
	fn serialize_char(mut self, v: char) -> Result<Self::Ok, Self::Error> {
		format!("{v}").encode(&mut self.writer).map_err(Self::Error::Fmt)?;
	...
	fn serialize_unit(mut self) -> Result<Self::Ok, Self::Error> {
		None::<i32>.encode(&mut self.writer).map_err(Self::Error::Fmt)?;
		self.writer.finish().map_err(Self::Error::Fmt)?;
		Ok(())
	}
	...
	fn serialize_unit_variant(self, _ty: &'static str, _index: u32, name: &'static str) -> Result<Self::Ok, Self::Error> {
		self.serialize_str(name)
	}

	fn serialize_newtype_struct<T>(self, _ty: &'static str, value: &T) -> Result<Self::Ok, Self::Error>
	where
		T: ?Sized + Serialize,
	{
	...
	fn serialize_tuple_struct(self, ty: &'static str, _len: usize) -> Result<Self::SerializeTuple, Self::Error> {
	...
	fn serialize_struct(self, ty: &'static str, _len: usize) -> Result<Self::SerializeStruct, Self::Error> {
	...
	where
		T: ?Sized + fmt::Display,
	{
		value.to_string().encode(&mut self.writer).map_err(Self::Error::Fmt)?;
