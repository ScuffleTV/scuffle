...
impl<G: ApiGlobal> PlaybackKeyPairServiceTrait for PlaybackKeyPairServer<G> {
	async fn get(&self, _request: Request<PlaybackKeyPairGetRequest>) -> Result<Response<PlaybackKeyPairGetResponse>> {
	...
	async fn tag(&self, request: Request<PlaybackKeyPairTagRequest>) -> Result<Response<PlaybackKeyPairTagResponse>> {
		let global = get_global(&self.global)?;

		let access_token = validate_auth_request(&global, &request, (Resource::PlaybackKeyPair, Permission::Modify)).await?;
	...
	async fn untag(&self, request: Request<PlaybackKeyPairUntagRequest>) -> Result<Response<PlaybackKeyPairUntagResponse>> {
		let global = get_global(&self.global)?;
