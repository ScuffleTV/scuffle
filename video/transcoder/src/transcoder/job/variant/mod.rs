...
pub async fn make_audio_stream(mut multi_stream: MultiStream<TrackOut>, listener: UnixListener) {
    multi_stream
        .run(pin!(track_parser(pin!(unix_stream(listener, 256 * 1024)))))
        .await
}

pub enum TrackSetup {
    Audio(MultiStreamSubscriber<TrackOut>),
    VideoAudio(UnixListener, MultiStreamSubscriber<TrackOut>),
}

impl TrackSetup {
    fn count(&self) -> u32 {
        match self {
            TrackSetup::Audio(_) => 1,
            TrackSetup::VideoAudio(_, _) => 2,
        }
    }
}
...
pub async fn handle_variant(
    ...
    variant_id: String,
    request_id: String,
    track_setup: TrackSetup,
) -> Result<String, ()> {
    let mut variant = Variant::new(track_setup.count(), stream_id, variant_id, request_id);

    match track_setup {
        TrackSetup::Audio(audio) => {
            variant
                .run(global, pin!(audio.into_stream().map(|r| (r, 1))))
                .await?;
        }
        TrackSetup::VideoAudio(video, audio) => {
            variant
                .run(
                    global,
                    tokio_stream::StreamExt::merge(
                        pin!(track_parser(pin!(unix_stream(video, 256 * 1024))).map(|r| (r, 1))),
                        pin!(audio.into_stream().map(|r| (r, 2))),
                    ),
                )
                .await?;
        }
            ...
            samples
                ...
                .map(|(idx, samples)| {
                    let mut traf = Traf::new(
                        ...
                        Some(Tfdt::new(
                            self.redis_state.track_duration(idx).unwrap() as u64
                        )),
                    );
        ...
        samples.iter().enumerate().for_each(|(idx, s)| {
            self.redis_state.set_track_duration(
                ...
                self.redis_state.track_duration(idx).unwrap()
                    + s.iter().map(|s| s.duration).sum::<u32>(),
            );
        });
