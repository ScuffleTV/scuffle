...
pub struct ChannelOpts {
	/// A list of addresses to connect to. If this is empty, will return an
	/// error. Can be a hostname or an IP address.
	pub addresses: Vec<String>,
	/// If true, will try to resolve CNAME records for the hostname.
	/// Useful for headless services. If false, will only resolve A/AAAA
	/// records.
	pub try_cname: bool,
	...
	/// How often to re-resolve the hostnames. If this is 0, will only resolve
	/// once.
...
impl<R: DnsResolver> ChannelController<R> {
	...
	async fn run(&mut self) -> bool {
		...
		future::join_all(futures).await.into_iter().for_each(|result| match result {
			...
			Ok((lookup, port)) => {
				lookup
					...
					// Since we are using a filter_map here, we can also filter out any records
					// that we don't care about.
					.filter_map(move |record| {
						match record {
							// If we get an A record back, we convert it to an SocketAddr with
							// the port and then into a EndpointType::Ip.
							RData::A(a) => Some(EndpointType::Ip(SocketAddr::new(a.0.into(), port))),
							// If we get an AAAA record back, we convert it to an SocketAddr
							// with the port and then into a EndpointType::Ip.
							RData::AAAA(aaaa) => Some(EndpointType::Ip(SocketAddr::new(aaaa.0.into(), port))),
							// If we get a CNAME record back, we convert it to an
							// EndpointType::CName with the port.
							RData::CNAME(cname) => Some(EndpointType::CName(cname.to_string(), port)),
							// This should never happen, but we have to handle it. We just
							// ignore it.
							_ => None,
						}
					})
		...
		for change in changes {
			// If this fails, it means the receiver has been dropped, so we can stop the
			// loop.
...
/// to services.
#[inline(always)]
pub fn make_channel(addresses: Vec<String>, interval: Duration, tls: Option<TlsSettings>) -> Result<Channel, GrpcError> {
...
pub fn make_channel_with_resolver<R: DnsResolver>(resolver: R, opts: ChannelOpts) -> Result<Channel, GrpcError> {
	...
	for address in opts.addresses {
		...
		let port = uri.port_u16().unwrap_or(if opts.tls.is_some() { 443 } else { 80 });
