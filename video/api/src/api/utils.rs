...
const TAG_ALPHABET: &str =
    r#"abcdefghijklmnopqrstuvwzyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-+?'";:[]{}"#;
...
pub async fn validate_auth_request<T>(
    ...
    let auth_string = String::from_utf8(auth)
        .map_err(|_| Status::unauthenticated("invalid authorization header"))?;
    ...
    let access_token_id = Ulid::from_str(access_token_id)
        .map_err(|_| Status::unauthenticated("invalid authorization header"))?;
    ...
    let secret_token = Ulid::from_str(secret_token)
        .map_err(|_| Status::unauthenticated("invalid authorization header"))?;
...
pub async fn add_tag_query(
    ...
    if let Some(organization_id) = organization_id {
        qb.push(" AND organization_id = ")
            .push_bind(Uuid::from(organization_id));
    }

    qb.push(" GROUP BY id) UPDATE ")
        .push(table)
        .push(" AS t")
        .push(
            r#"
    ...
    row.map(|row| match row.status {
        0 | 1 => Ok(row.tags.0),
        2 => Err(Status::invalid_argument(
            "tags must not exceed 15".to_string(),
        )),
        _ => unreachable!(),
    })
    .transpose()
