use pb::{ext::UlidExt, scuffle::video::v1::AccessTokenGetRequest};
use tonic::Status;
use uuid::Uuid;
        ...
        if let Some(tags) = options.tags.as_ref() {
            if !tags.tags.is_empty() {
                ...
                seperated.push_bind_unseparated(
                    serde_json::to_value(&tags.tags)
                        .map_err(|_| Status::internal("failed to serialize tags"))?,
                );
            }
        }
...
mod test {
        ...
        let access_token_get_request = AccessTokenGetRequest {
            ...
            search_options: Some(SearchOptions {
                ...
                tags: Some(pb::scuffle::video::v1::types::Tags {
                    tags: vec![(String::from("test"), String::from("test"))]
                        .into_iter()
                        .collect(),
                }),
            }),
    ...
    fn test_query_build_with_tags_and_after_id() {
        ...
        let tags = Tags {
            tags: vec![(String::from("key"), String::from("value"))]
                .into_iter()
                .collect(),
        };
