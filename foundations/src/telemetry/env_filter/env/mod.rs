...
pub use self::{builder::Builder, directive::Directive};
mod builder;
mod directive;
mod field;
...
use tracing::{callsite, span, Metadata};
...
/// directives.
///
/// `EnvFilter` implements both the [`Layer`](#impl-Layer<S>) and [`Filter`]
/// traits, so it may be used for both [global filtering][global] and [per-layer
/// filtering][plf], respectively. See [the documentation on filtering with
/// `Layer`s][filtering] for details.
///
...
/// # Directives
///
/// A filter consists of one or more comma-separated directives which match on
/// [`Span`]s and [`Event`]s. Each directive may have a corresponding maximum
/// verbosity [`level`] which enables (e.g., _selects for_) spans and events
/// that match. Like `log`, `tracing` considers less exclusive levels (like
/// `trace` or `info`) to be more verbose than more exclusive levels (like
/// `error` or `warn`).
///
/// The directive syntax is similar to that of [`env_logger`]'s. At a high
/// level, the syntax for directives consists of several parts:
///
/// ```text
/// target[span{field=value}]=level
/// ```
///
/// Each component (`target`, `span`, `field`, `value`, and `level`) will be
/// covered in turn.
///
/// - `target` matches the event or span's target. In general, this is the
///   module path and/or crate name. Examples of targets `h2`, `tokio::net`, or
///   `tide::server`. For more information on targets, please refer to
///   [`Metadata`]'s documentation.
/// - `span` matches on the span's name. If a `span` directive is provided
///   alongside a `target`, the `span` directive will match on spans _within_
///   the `target`.
/// - `field` matches on [fields] within spans. Field names can also be supplied
///   without a `value` and will match on any [`Span`] or [`Event`] that has a
///   field with that name. For example: `[span{field=\"value\"}]=debug`,
///   `[{field}]=trace`.
/// - `value` matches on the value of a span's field. If a value is a numeric
///   literal or a bool, it will match _only_ on that value. Otherwise, this
///   filter matches the [`std::fmt::Debug`] output from the value.
/// - `level` sets a maximum verbosity level accepted by this directive.
///
/// When a field value directive (`[{<FIELD NAME>=<FIELD_VALUE>}]=...`) matches
/// a value's [`std::fmt::Debug`] output (i.e., the field value in the directive
...
/// ## Usage Notes
///
/// - The portion of the directive which is included within the square brackets
///   is `tracing`-specific.
/// - Any portion of the directive can be omitted.
///     - The sole exception are the `field` and `value` directives. If a
///       `value` is provided, a `field` must _also_ be provided. However, the
///       converse does not hold, as fields can be matched without a value.
/// - If only a level is provided, it will set the maximum level for all `Span`s
///   and `Event`s that are not enabled by other filters.
/// - A directive without a level will enable anything that it matches. This is
///   equivalent to `=trace`.
/// - When a crate has a dash in its name, the default target for events will be
///   the crate's module path as it appears in Rust. This means every dash will
...
///    - are within the `span_a` span or named `span_a` _if_ `span_a` has the
///      target `my_crate`,
...
pub struct EnvFilter {
	...
	has_dynamics: bool,
	by_id: scc::HashMap<span::Id, directive::SpanMatcher>,
	by_cs: scc::HashMap<callsite::Identifier, directive::CallsiteMatcher>,
...
impl EnvFilter {
	...
	pub fn enabled<S>(&self, metadata: &Metadata<'_>, _: Context<'_, S>) -> bool {
		...
		if self.has_dynamics && self.dynamics.max_level >= *level {
			if metadata.is_span() {
				...
				let enabled_by_cs = self.by_cs.contains(&metadata.callsite());
				if enabled_by_cs {
					return true;
				}
			}
	...
	/// [`Filter::max_level_hint`] methods on `EnvFilter`'s implementations of
	/// those traits, but it does not require the trait to be in scope.
	///
	...
	pub fn max_level_hint(&self) -> Option<LevelFilter> {
		...
		std::cmp::max(self.statics.max_level.into(), self.dynamics.max_level.into())
	}

	/// Informs the filter that a new span was created.
	///
	/// This is equivalent to calling the [`Layer::on_new_span`] or
	/// [`Filter::on_new_span`] methods on `EnvFilter`'s implementations of
	/// those traits, but it does not require the trait to be in scope.
	pub fn on_new_span<S>(&self, attrs: &span::Attributes<'_>, id: &span::Id, _: Context<'_, S>) {
		...
		if let Some(cs) = self.by_cs.get(&attrs.metadata().callsite()) {
			let span = cs.get().to_span_match(attrs);
			self.by_id
				.insert(id.clone(), span)
				.expect("a span should only be registered once");
		}
	}
	...
	pub fn on_enter<S>(&self, id: &span::Id, _: Context<'_, S>) {
		...
		if let Some(span) = self.by_id.get(id) {
			self.scope.get_or_default().borrow_mut().push(span.get().level());
		}
	}
	...
	pub fn on_record<S>(&self, id: &span::Id, values: &span::Record<'_>, _: Context<'_, S>) {
		...
		if let Some(span) = self.by_id.get(id) {
			span.get().record_update(values);
		}
	}

	fn cares_about_span(&self, span: &span::Id) -> bool {
		self.by_id.contains(span)
	}
	...
	fn register_callsite(&self, metadata: &'static Metadata<'static>) -> Interest {
		if self.has_dynamics && metadata.is_span() {
			...
			if let Some(matcher) = self.dynamics.matcher(metadata) {
				self.by_cs
					.insert(metadata.callsite(), matcher)
					.expect("a callsite should only be registered once");
...
mod tests {
	...
	fn callsite_enabled_no_span_directive() {
		let filter = EnvFilter::new("app=debug").with_subscriber(Registry::default());
	...
	#[test]
	fn callsite_off() {
		let filter = EnvFilter::new("app=off").with_subscriber(Registry::default());
		static META: &Metadata<'static> = &Metadata::new(
			"mySpan",
			"app",
			Level::ERROR,
	...
	fn callsite_enabled_includes_span_directive() {
		let filter = EnvFilter::new("app[mySpan]=debug").with_subscriber(Registry::default());
		static META: &Metadata<'static> = &Metadata::new(
			"mySpan",
			"app",
			Level::TRACE,
	...
	fn callsite_enabled_includes_span_directive_field() {
		let filter = EnvFilter::new("app[mySpan{field=\"value\"}]=debug").with_subscriber(Registry::default());
		static META: &Metadata<'static> = &Metadata::new(
			"mySpan",
			"app",
			Level::TRACE,
	...
	fn callsite_enabled_includes_span_directive_multiple_fields() {
		let filter = EnvFilter::new("app[mySpan{field=\"value\",field2=2}]=debug").with_subscriber(Registry::default());
		static META: &Metadata<'static> = &Metadata::new(
			"mySpan",
			"app",
			Level::TRACE,
	...
	#[test]
	fn roundtrip() {
		let f1: EnvFilter = "[span1{foo=1}]=error,[span2{bar=2 baz=false}],crate2[{quux=\"quuux\"}]=debug"
			.parse()
			.unwrap();
	...
	fn size_of_filters() {
		fn print_sz(s: &str) {
			...
			println!("size_of_val({:?})\n -> {}B", s, std::mem::size_of_val(&filter));
		}
