...
use super::{EnvFilter, FromEnvError};
...
impl Builder {
	...
	/// Returns a new [`EnvFilter`] from the directives in the configured
	/// environment variable. If the environment variable is unset, no directive
	/// is added.
	///
	...
	pub(super) fn from_directives(&self, directives: impl IntoIterator<Item = Directive>) -> EnvFilter {
		use tracing::Level;
		...
		if !disabled.is_empty() {
			...
			let help_msg = || {
				let (feature, filter) = match STATIC_MAX_LEVEL.into_level() {
					Some(Level::TRACE) => {
						unreachable!("if the max level is trace, no static filtering features are enabled")
					}
