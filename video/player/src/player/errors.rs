...
impl<T> EventErrorExtFetch<T> for FetchResult<T> {
    fn into_event_error(self, fatal: bool) -> Result<T, EventError> {
        self.map_err(|err| match err {
            FetchError::Aborted => {
                EventError::new(ErrorCode::Network, "request was aborted".into(), fatal)
            }
            FetchError::EmptyResponse => {
                EventError::new(ErrorCode::Network, "response was empty".into(), fatal)
            }
            FetchError::InvalidResponse => {
                EventError::new(ErrorCode::Network, "response was invalid".into(), fatal)
            }
            FetchError::JsValue(e) => {
                EventError::new(ErrorCode::Network, "Javascript Error".into(), fatal).with_source(e)
            }
            FetchError::Json(e) => EventError::new(
                ErrorCode::Network,
