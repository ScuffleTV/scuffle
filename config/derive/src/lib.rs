...
fn impl_config(ast: &syn::DeriveInput) -> syn::Result<TokenStream> {
	...
	let struct_env_attr = attributes
		.iter()
		.find_map(|a| if let Attr::Env(e) = a { Some(e) } else { None });

	let struct_cli_attr = attributes
		.iter()
	...
	for field in fields.named.iter() {
		let Some(ident) = &field.ident else {
			return Err(syn::Error::new_spanned(field, "Only named fields are supported"));
		};
		...
		let type_attr = attributes
			.iter()
			.find_map(|a| if let Attr::KeyType(e) = a { Some(e) } else { None })
			.map(|t| quote! { #t })
		...
		keys_init.push(if flatten {
			...
			let add_attrs = {
				let env_attr =
					field_env_attr.and_then(|env_attr| env_attr.skip.then_some(quote! { let key = key.with_skip_env(); }));

				let cli_attr =
					field_cli_attr.and_then(|cli_attr| cli_attr.skip.then_some(quote! { let key = key.with_skip_cli(); }));
...
fn get_attributes(attrs: &[syn::Attribute]) -> syn::Result<Vec<Attr>> {
	attrs
		...
		.map(|attr| {
			// the syntax looks like #[config(default = "true")] or #[config(from_str =
			// "parse")] or #[config(from_str)] or #[config(default)]
					...
					if path.is_ident("graph") {
						...
						match value {
							syn::Expr::Lit(syn::ExprLit {
								lit: syn::Lit::Str(lit), ..
							}) => Ok(Attr::KeyType(syn::parse_str(&lit.value())?)),
...
fn get_doc_comment(attrs: &[syn::Attribute]) -> Option<String> {
	attrs.iter().find(|a| a.path().is_ident("doc")).and_then(|a| {
