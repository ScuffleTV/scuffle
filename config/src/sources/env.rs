...
/// Environment source
///
/// Create a new environment source with [`EnvSource::new`](EnvSource::new),
/// [`EnvSource::with_prefix`](EnvSource::with_prefix) or
/// [`EnvSource::with_joiner`](EnvSource::with_joiner).
	...
	pub fn with_joiner(prefix: Option<&str>, joiner: &str) -> Result<Self> {
		Ok(Self {
			...
			value: extract_keys(
				&C::graph(),
				prefix,
				prefix.map(|p| KeyPath::root().push_struct(p)).unwrap_or_default(),
				joiner,
				false,
				false,
			)
			.and_then(|val| C::transform(&KeyPath::root(), val.unwrap_or_else(|| Value::Map(BTreeMap::new()))))
			.map_err(|e| {
...
fn extract_keys(
	...
	match graph {
		...
		| KeyGraph::Unit => {
			...
			if seq {
				Ok(Some(Value::Seq(
					value.split(',').map(|s| Value::String(s.to_string())).collect(),
				)))
			} else {
		...
		KeyGraph::Struct(map) => {
			...
			let result = map
				...
				.filter_map(|(child_path, key)| {
					...
					Some(
						extract_keys(key.graph(), prefix, path.push_struct(child_path), joiner, false, false)
		...
		KeyGraph::Option(graph) => {
			if seq {
				return Err(ConfigError::new(ConfigErrorType::UnsupportedType(graph.clone())).with_path(path));
			}
