...
use super::{
    define,
    define::{RtmpVersion, SchemaVersion, ServerHandshakeState},
...
impl SimpleHandshakeServer {
    ...
    fn write_s2(&self, writer: &mut BytesWriter) -> Result<(), HandshakeError> {
        // Time (4 bytes): This field MUST contain the timestamp sent by the C1 (for S2).
...
impl ComplexHandshakeServer {
    ...
    fn read_c1(&mut self) -> Result<(), HandshakeError> {
        let c1_bytes = self
            .reader
            .read_bytes(define::RTMP_HANDSHAKE_SIZE)?
            .freeze();
    ...
    fn write_s1(&self, main_writer: &mut BytesWriter) -> Result<(), HandshakeError> {
        ...
        // Note: this is not a security issue since we do not flush the buffer until we are done
        //  with the handshake.
    ...
    fn write_s2(&self, main_writer: &mut BytesWriter) -> Result<(), HandshakeError> {
        ...
        // define::RTMP_HANDSHAKE_SIZE - define::TIME_VERSION_LENGTH because we already wrote 8 bytes. (timestamp and c1 timestamp)
        for _ in 0..define::RTMP_HANDSHAKE_SIZE - define::TIME_VERSION_LENGTH {
            writer.write_u8(rng.gen())?;
        }
        ...
        // Create a digest of the random data using a key generated from the digest of C1.
        let data_digest =
            DigestProcessor::new(Bytes::new(), key_digest.make_digest(&self.c1_digest, &[])?);
...
impl Default for HandshakeServer {
    fn default() -> Self {
        Self {
            ...
            // We attempt to do a complex handshake by default. If the client does not support it, we fallback to simple.
            is_complex: true,
