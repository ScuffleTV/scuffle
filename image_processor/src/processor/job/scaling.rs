...
impl ScalingOptions {
	pub fn compute(&mut self) -> Vec<Size<usize>> {
		...
		let (best_idx, input_scale_factor) = scales
			.iter()
			.position(|(size, _)| size.width >= padded_size.width || size.height >= padded_size.height)
			.map(|idx| (idx, Ratio::ONE))
			.unwrap_or_else(|| {
				let size = scales.last().unwrap().0;

				// Since its the padded size, the aspect ratio is the same as the target aspect
				// ratio.
		...
		if self.upscale.is_no() {
			scales.retain(|(size, _)| size.width <= padded_size.width && size.height <= padded_size.height);
		}
...
mod tests {
	...
	fn test_compute_scales_different_aspect() {
		...
		options.upscale = Upscale::NoPreserveSource;
		assert_eq!(options.compute(), vec![Size { width: 178, height: 100 },]);
		...
		options.upscale = Upscale::NoPreserveSource;
		assert_eq!(options.compute(), vec![Size { width: 100, height: 178 },]);
