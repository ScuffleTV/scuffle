...
pub fn wrapped(args: TokenStream, input: TokenStream) -> syn::Result<proc_macro2::TokenStream> {
	...
	let self_dot = if has_self_argument { quote!(self.) } else { quote!() };
	...
	input.attrs = vec![
		...
		// we also don't want the inner function to appear in docs or autocomplete (if they do, they should be deprecated
		// and give a warning if they are used)
		parse_quote!(#[doc(hidden)]),
		parse_quote!(#[deprecated = "internal wrapper function, please do not use!"]),
		parse_quote!(#[inline(always)]), /* let's make sure we don't produce more overhead than we need to, the output
		                                  * should produce similar assembly to the input (besides the end) */
	];

	// for functions that take a self argument, we will need to put the inner
	// function outside of our new function since we don't know what type self is
