use super::{
    define::{
        Chunk, ChunkBasicHeader, ChunkMessageHeader, ChunkType, INIT_CHUNK_SIZE, MAX_CHUNK_SIZE,
    },
    errors::ChunkDecodeError,
};
...
// These constants are used to limit the amount of memory we use for partial chunks
// on normal operations we should never hit these limits
...
pub struct ChunkDecoder {
    ...
    /// According to the spec chunk streams are identified by the chunk stream ID.
    /// In this case that is our key.
    /// We then have a chunk header (since some chunks refer to the previous chunk header)
    previous_chunk_headers: HashMap<u32, ChunkMessageHeader>,

    /// Technically according to the spec, we can have multiple message_streams in a single chunk_stream
    /// Because of this we actually have to have a map of chunk streams to message streams to bytes
    /// The first u32 is the chunk stream id, the second is the message stream id
...
impl ChunkDecoder {
    ...
    pub fn read_chunk(&mut self) -> Result<Option<Chunk>, ChunkDecodeError> {
        ...
        // And those chunks may be partial chunks thus we need to keep reading until we have
        // a full chunk or we run out of data.
        loop {
            ...
            // Since we were reading from an advanced cursor, our reads did not actually advance the
            // reader's position. We need to manually advance the reader's position to the cursor's
            // position.
            ...
            // Data before the slice is the header data, and data after the slice is the next chunk
            // We don't need to keep the header data, because we already decoded it into struct form.
            // The payload_range_end should be the same as the cursor's position.
            ...
            // This isnt a spec check but it is a check to make sure that we dont have too many
            // previous chunk headers stored in memory.
            let count = if self
                .previous_chunk_headers
                .contains_key(&header.chunk_stream_id)
            {
            ...
            // If this is hit, then we have too many previous chunk headers stored in memory.
            // And the client is probably trying to DoS us.
            ...
            // It is possible in theory to get a chunk message that requires us to change the max chunk size.
            // However the size of that message is smaller than the default max chunk size.
            // Therefore we can ignore this case.
            // Since if we get such a message we will read it and the payload.len() will be equal to the message length.
            // and thus we will return the chunk.
            ...
            if payload.len() == message_header.msg_length as usize {
                ...
                // Otherwise we generate a key using the chunk stream id and the message stream id.
                // We then get the partial chunk from the map using the key.
                ...
                let length = {
                    ...
                    if partial_chunk.len() + payload.len() > MAX_PARTIAL_CHUNK_SIZE {
                        return Err(ChunkDecodeError::PartialChunkTooLarge(
                ...
                // Usually this will result in returning Ok(None) from one of the above checks.
                // However there is a edge case that we have enough data in our buffer to read the
                // next chunk and the client is waiting for us to send a response. Meaning if we just return Ok(None) here
                // We would deadlock the connection, and it will eventually timeout.
                // So we need to loop again here to check if we have enough data to read the next chunk.
            }
        }
    }
    ...
    fn read_header(
    ...
    ) -> Result<ChunkBasicHeader, Option<ChunkDecodeError>> {
        // The first byte of the basic header is the format of the chunk and the stream id.
        // Mapping the error to none means that this isn't a real error but we dont have enough data.
        let byte = cursor.read_u8().map_err(|_| None)?;
        // The format is the first 2 bits of the byte. We shift the byte 6 bits to the right to get the format.
        let format = (byte >> 6) & 0b00000011;

        // We check that the format is valid.
        // Since we do not map to None here, this is a real error and the connection will be closed.
        // It should not be possible to get an invalid chunk type because, we bitshift the byte 6 bits to the right.
        // Leaving 2 bits which can only be 0, 1 or 2 or 3 which is the only valid chunk types.
        let format =
            ChunkType::from_u8(format).ok_or(ChunkDecodeError::InvalidChunkType(format))?;
        ...
        let chunk_stream_id = match (byte & 0b00111111) as u32 {
            ...
            // If it is 1 we read the next 2 bytes and add 64 to it and multiply the 2nd byte by 256.
            1 => {
                64 + cursor.read_u8().map_err(|_| None)? as u32
                    + cursor.read_u8().map_err(|_| None)? as u32 * 256
            }
    ...
    fn read_message_header(
        ...
        match header.format {
            ...
            ChunkType::Type0 => {
                ...
                // Followed by a 3 byte message length. (this is the length of the entire payload not just this chunk)
                let msg_length = cursor.read_u24::<BigEndian>().map_err(|_| None)?;
                if msg_length as usize > MAX_PARTIAL_CHUNK_SIZE {
                    return Err(Some(ChunkDecodeError::PartialChunkTooLarge(
                        msg_length as usize,
                    )));
                }
                ...
                // We validate the message type id. If it is invalid we return an error. (this is a real error)
                let msg_type_id = MessageTypeID::from_u8(msg_type_id)
                    .ok_or(ChunkDecodeError::InvalidMessageTypeID(msg_type_id))?;

                // We then read the message stream id. (According to spec this is stored in LittleEndian, no idea why.)
                ...
                // Why not just use 3 bytes for the timestamp, and if the 3 bytes are set to 0xFFFFFF just read 1 additional byte and then shift it 24 bits.
                // Like if timestamp == 0xFFFFFF { timestamp |= cursor.read_u8().map_err(|_| None)? << 24; }
                // This would save 3 bytes in the header and would be more efficient but I guess the Spec writers are smarter than me.
            ...
            ChunkType::Type1 => {
                ...
                // Again as mentioned above we sometimes have a delta timestamp larger than 3 bytes.
                ...
                // If the previous chunk header is not found we return an error. (this is a real error)
                let previous_header = self
                    .previous_chunk_headers
                    .get(&header.chunk_stream_id)
                    .ok_or(ChunkDecodeError::MissingPreviousChunkHeader(
                        header.chunk_stream_id,
                    ))?;

                // We calculate the timestamp by adding the delta timestamp to the previous timestamp.
                // We need to make sure this does not overflow.
            ...
            // The message length, message type id and message stream id are the same as the previous chunk.
            ChunkType::Type2 => {
                // We read the delta timestamp.
                let timestamp_delta = cursor.read_u24::<BigEndian>().map_err(|_| None)?;

                // Again if the delta timestamp is larger than 3 bytes we read the next 4 bytes as the timestamp.
                let (timestamp_delta, was_extended_timestamp) = if timestamp_delta == 0xFFFFFF {
                    (cursor.read_u32::<BigEndian>().map_err(|_| None)?, true)
                } else {
                    (timestamp_delta, false)
            ...
            ChunkType::Type3 => {
                ...
                // If the PREVIOUS HEADER is extended then we now waste an additional 4 bytes to read the timestamp.
                // Why not just read the timestamp in the previous header if it is extended?
                // I guess the spec writers had some reason and its obviously way above my knowledge.
    ...
    fn get_payload_range(
        ...
        // We find out if the chunk is a partial chunk (and if we have already read some of it).
        let key = (header.chunk_stream_id, message_header.msg_stream_id);

        // Check how much we still need to read (if we have already read some of the chunk)
