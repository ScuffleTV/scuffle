...
use self::{
    blank::VideoFactory,
    track::TrackState,
    utils::{make_video_element_holder, MediaSourceEvent, VideoElementEvent},
    visibility_detector::VisibilityDetector,
};

use super::{
    api::{ApiMediaClient, ApiSessionClient, Json},
    inner::{PlayerInnerHolder, PlayerState},
    util::{now, Holder},
};
...
pub struct Runner {
    ...
    /// A video factory is used to generate blank video frames when there is no video track.
    ...
    /// A temporary buffer for video frames, this is used when we are switching video tracks.
    temporary_video_buffer: Vec<Bytes>,

    /// A temporary buffer for audio frames, this is used when we are switching audio tracks.
    /// We also need to store the timestamp and duration of the audio frame.
    temporary_audio_buffer: Vec<(Bytes, u64, u32)>,

    /// A holder for the source buffers.
    source_buffers: Option<source_buffer::SourceBuffers>,

    /// The visibility detector is used to detect if the video element is visible or not.
    visibility_detector: VisibilityDetector,
