...
async fn handle_room_event<G: ApiGlobal>(global: &Arc<G>, event: event::Room, timestamp: i64) -> anyhow::Result<()> {
	...
	match event.event.context("no event")? {
		event::room::Event::Ready(event::room::Ready {
			...
			let channel_id = scuffle_utils::database::query("UPDATE users SET channel_active_connection_id = $1, channel_live_viewer_count = $2, channel_live_viewer_count_updated_at = NOW(), channel_last_live_at = $3 WHERE channel_room_id = $4 RETURNING id")
		...
		event::room::Event::Disconnected(event::room::Disconnected {
			...
			let res = scuffle_utils::database::query("UPDATE users SET channel_active_connection_id = NULL, channel_live_viewer_count = 0, channel_live_viewer_count_updated_at = NOW() WHERE channel_room_id = $1 AND channel_active_connection_id = $2 RETURNING id")
				.bind(room_id.into_ulid())
				.bind(connection_id.into_ulid())
				.build_query_single_scalar()
