...
#[derive(Clone, Debug)]
pub struct HashMapCache<L: Loader<S>, S = RandomState, S2 = S>(std::collections::HashMap<L::Key, L::Value, S2>);

impl<L: Loader<S>, S, S2: std::hash::BuildHasher> AutoImplCacheMutRef<L, S> for HashMapCache<L, S, S2> {}
...
// This is a compile time test to ensure that the traits are implemented
// correctly.
const _: () = {
	struct DummyLoader;
	...
	assert_auto_impl_cache_ref::<&SharedCache<HashMapCache<DummyLoader>>, DummyLoader, RandomState>();
	assert_auto_impl_cache_ref::<&mut SharedCache<HashMapCache<DummyLoader>>, DummyLoader, RandomState>();

	assert_auto_impl_cache_ref::<SharedCache<SharedCache<HashMapCache<DummyLoader>>>, DummyLoader, RandomState>();
	assert_auto_impl_cache_ref::<&SharedCache<SharedCache<HashMapCache<DummyLoader>>>, DummyLoader, RandomState>();
	assert_auto_impl_cache_ref::<&mut SharedCache<SharedCache<HashMapCache<DummyLoader>>>, DummyLoader, RandomState>();
